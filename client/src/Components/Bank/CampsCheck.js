                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td className="text-right">
                            {edit ? (
                                <>
                                    <i className="fa-solid text-metal fa-circle-info" onClick={() => { 
                                        props.setPopup(props.popup === -1 ? 1 : -1); 
                                        props.setSent(props.data._id); 
                                    }}></i> &nbsp;
                                    {status === 0 && (
                                        <i className="fa-solid text-green fa-pen-to-square" onClick={() => setEdit(false)}></i>
                                    )}
                                </>
                            ) : (
                                <>
                                    <i className="fa-solid text-green fa-check" onClick={async () => {
                                        try {
                                            await axios.put(`/camps/${props.camp}/${props.data._id._id}`, { units }, { withCredentials: true });
                                            alert("Updated");
                                    ...
                                    <i className="fa-solid fa-xmark text-blood" onClick={() => setEdit(true)}></i>
                                </>
                            )}
                        </td>
                    </tr>
